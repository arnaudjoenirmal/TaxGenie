...
#     "model": "llama3-70b-8192",
#     "messages": [
#         {"role": "system", "content": "Extract key-value pairs, transactions, and summary in valid JSON format."},
#         {"role": "user", "content": json.dumps(input_data)}
#     ],
#     "response_format": "json",  # Ensure response is JSON
...
#             error_detail = f"Async request failed: {str(e)}"
#             logger.error(error_detail)
#             raise HTTPException(status_code=500, detail=error_detail)
...
# Load API key from environment variable (ensure this is set in production)
...
def process_groq_data(input_data: dict):
    """
    Sends input data to the Groq API synchronously and returns the processed response.
    """
    headers = {
        "Authorization": f"Bearer {GROQ_API_KEY}",
        "Content-Type": "application/json"
    ...
    payload = {
        "model": "llama3-70b-8192",
        "messages": [
            {"role": "system", "content": "Extract key-value pairs, transactions, and a financial summary in valid JSON."},
            {"role": "user", "content": user_content}
        ],
        "response_format": "json_object",  # Ensure Groq API returns JSON format
        "temperature": 0.0,
        "max_tokens": 2000
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Groq API Response: {json.dumps(response_data, indent=2)}")

            # Extract and return structured content
            if "choices" in response_data and response_data["choices"]:
                return json.loads(response_data["choices"][0]["message"]["content"])  # Ensure it's JSON
        ...
        logger.error("Failed to parse Groq API response as JSON")
...
async def process_groq_data_async(input_data: dict):
    """
    Asynchronously sends input data to the Groq API and returns the processed response.
    """
    headers = {
        "Authorization": f"Bearer {GROQ_API_KEY}",
        "Content-Type": "application/json"
