...
def process_groq_data(input_data: dict):
    ...
    # Modified system prompt to specifically request JSON output
    system_prompt = """
    Extract the financial data from the provided information and return it as a properly formatted JSON object with the following structure:
    {
        "accountDetails": {
            "accountNo": "string",
            "printDate": "string",
    ...
    try:
        ...
        # Extract only the content from the assistant's message
        if "choices" in response_json and len(response_json["choices"]) > 0:
            content = response_json["choices"][0]["message"]["content"]
            
            # Try to parse the content as JSON
            try:
                # If the content already contains JSON, extract it
                financial_data = json.loads(content)
                return financial_data
            except json.JSONDecodeError:
                # If there's text around the JSON, try to extract just the JSON part
                ...
                if json_match:
                    ...
                    raise HTTPException(status_code=500, detail="No JSON content found in response")
        else:
